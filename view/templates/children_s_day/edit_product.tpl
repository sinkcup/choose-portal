...
<div class="module_filter">
    ...
    <form method="post" action="./save">
                ...
                foreach($v['filter_rows'] as $row)
                {
                    ...
                    if(!is_array($row))
                    {
                        ...
                        foreach($v['attribute_value'][$attribute_name] as $value=>$value_display)
                        {
                            ...
                            echo $value==$v['product'][$attribute_name] ? ' checked="checked"' : '';
                            echo ' />';
                    ...
                    else
                    {
                        ...
                        foreach($tmp[0] as $attribute_name)
                        {
                            ...
                            echo 1==$v['product'][$attribute_name] ? ' checked="checked"' : '';
                            echo ' />';
        ...
        {
            echo '<div><img height="160" src="' . FdHelperLib::getImgUri($one, 'auto', '160') . '" alt="" />';
            echo '<input type="text" name="preview_img[]" value="' . $one . '" />';
            echo '<a href="#" onclick="this.parentNode.parentNode.removeChild(this.parentNode);return false;">删除</a>';
            echo '</div>';
        }
        ?>
        </div>
